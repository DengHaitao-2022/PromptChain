...
class Uncertainty(BaseModel):
    ...
    default_assumption: Optional[str] = Field(None, description="系统默认假设")
...
class IntentCard(BaseModel):
    ...
    scenario: Optional[str] = Field(None, description="使用场景：如公众号、演讲稿、报告")
    ...
    def get_high_priority_uncertainties(self, max_count: int = 3) -> List[Uncertainty]:
        ...
        return sorted(
            [u for u in self.uncertainties if u.priority <= 2],
