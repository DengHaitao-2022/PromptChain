...
def needs_schema_instruction(effective_method: str | None) -> bool:
    """json_mode / json_schema 都补一层 schema 指令，降低兼容实现漂移。"""
    return effective_method in {"json_mode", "json_schema"}
...
def ensure_json_mode_instruction_text(
    ...
    *,
    ...
    """仅在需要 schema 约束时追加 JSON 指令，并避免重复追加。"""
    if not needs_schema_instruction(effective_method):
...
def build_structured_chat_prompt(
    ...
    *,
    ...
    return ChatPromptTemplate.from_messages(
        [
            SystemMessage(content=_build_json_mode_instruction(schema)),
