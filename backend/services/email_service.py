...
from models.auth_orm import EmailVerificationTokenORM, PasswordResetTokenORM, UserORM
...
class EmailService:
    ...
    async def send_email(
        ...
        if not SMTP_USER or not SMTP_PASSWORD:
            # 开发环境：打印邮件内容而不是实际发送
            print(f"[DEV] 邮件发送到 {to_email}")
            print(f"[DEV] 主题: {subject}")
            print(f"[DEV] 内容: {html_content}")
            return True
    ...
    async def verify_email_token(self, token: str) -> Optional[str]:
        ...
        result = await self.session.execute(
            select(EmailVerificationTokenORM).where(
                and_(
                    ...
                    EmailVerificationTokenORM.expires_at > datetime.utcnow()
                )
            )
        )
        ...
        # 标记为已使用
        token_orm.used_at = datetime.utcnow()
        await self.session.commit()
        ...
        result = await self.session.execute(
            select(PasswordResetTokenORM).where(
                and_(
                    ...
                    PasswordResetTokenORM.expires_at > datetime.utcnow()
                )
            )
        )
        token_orm = result.scalar_one_or_none()
