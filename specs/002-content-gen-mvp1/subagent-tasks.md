...
- 当前派工事实源固定为 `dev@df88a42`、[tasks.md](./tasks.md) 顶部状态快照，以及 canonical 协作文件。
...
| Quickstart / Contracts / 协作文档 | `T043`, `T044`, `T047`, `T048` | 本轮已回填 | 文档现已按 `dev@df88a42` 对齐，不再沿用旧任务表结论 |
| 验收与 polish | `T017`, `T024`, `T031`, `T042`, `T045`, `T046` | 仍待继续 | 这些项主要是最终验收、文案与错误路径收口，不是“主线完全没有代码” |
...
- 当前 MVP1 不存在仍在推进中的核心实现分支；下一轮若继续开发，应直接从 `dev@df88a42` 新建干净分支。
- 根工作区中的 `.cunzhi-memory/*`、`backend/orm/`、`specs/002-content-gen-mvp1/*.jsonl`、`update.py` 属于本地运行态或未入主线内容，不能当作“已经进入 dev”的功能证据。
...
- 前端任何实际编码仍必须由 Gemini CLI 在独立 `git worktree` 中完成，并记录到 `gemini-executions.jsonl`。
